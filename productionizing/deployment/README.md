# Production NiFi Stack
A production-ready fullstack deployment of NiFi ecosystem. 
...
## Docker Builds
All Docker images are pulled from Docker hub registry with an exception of NiFi, NiFi Registry and NiFi Ceritificate Authority (CA) images which are built from this repository (nifi, nifi-ca, nifi-registry).
...
```bash
# Export the environment variables such as OIDC Discovery URL, Client ID and Client Secret
# Make sure you provide the client id and secret before running the below command
$ source ./env.sh
